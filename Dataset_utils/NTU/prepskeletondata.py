...
save_npy_path = '/net/polaris/storage/deeplearning/ntu/nturgb+d_skeletons_npy120/'
load_txt_path = '/net/polaris/storage/deeplearning/ntu/test3/'
...
def _read_skeleton(file_path, save_skelxyz=True, save_rgbxy=True, save_depthxy=True):
    ...
    max_body_count = max(bodymat['nbodys']) if bodymat['nbodys'] else 0
    for each in range(max_body):
        if each >= max_body_count:
            if save_skelxyz:
